...
//
RentalStore::RentalStore(string aName) : name(aName) {}//end name Constructor
...
//
RentalStore::RentalStore(const RentalStore& rightSide) {}//end copy constructor

//----------------------------------------------------------------------------
// Destructor
// destructor of store item for proper memory management
//  
RentalStore::~RentalStore() {}//end destructor
...
//
void RentalStore::createCustomers(istream& infile){	
...
//
void RentalStore::createInventory(istream& infile){		
...
// @param infile
//
void RentalStore::processTransactions(istream& infile) {
	char actCh = ' ';
	char movCh = ' ';
	char medCh = ' ';
	int custID = 0;
...
// out.
//
void RentalStore::displayInventory() const {
	if (name != ""){
		cout 
		<< "--------------------------------------------------------------"
		<< endl;
...
// 
void RentalStore::displayCustomerBase() const {
...
//
bool RentalStore::lookUpCustomer(int IDNumber) const {
...
//
string RentalStore::getBusinessName() const {
	return name;
}//end getBusinessName
...
Customer RentalStore::getCustomer(const int IDNumber) const {
